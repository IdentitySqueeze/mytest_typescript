...
describe("utils test", function () {
    ...
    function run_func_until_not(not_us, func, max_tries, ...rest) {
        var rtn = not_us[0];
        let i = 0;
        while (rtn in not_us) {
            rtn = func(rest);
    ...
    function not_bookend_biased(x, y, func) {
        if (x - y == 1 || y - x == 1)
            return true;
        return run_func_until_not([x, y], func, 250, [x, y]);
    }
    it("ri test", function () {
        function test_ri(x, y, bott, top) {
    ...
    it("rd test", function () {
        function test_rd(x, y, z) {
            let rslt = utils.mytest_rd(x, y, z);
    ...
    it("int choose test", function () {
        ...
        assert.oneOf(utils.mylib_choose([2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 8192, 16384, 32768, 65536]), [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 8192, 16384, 32768, 65536]);
    });
    ...
    it("to sub test (DISABLED)", function () {
        return true;
        assert.equal(utils.mylib_toSub("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-=()"), "ₐ₈CDₑբ₉ₕᵢⱼₖₗₘₙₒₚQᵣₛₜᵤᵥwₓᵧZₐ♭꜀ᑯₑբ₉ₕᵢⱼₖₗₘₙₒₚ૧ᵣₛₜᵤᵥwₓᵧ₂₀₁₂₃₄₅₆₇₈₉₊₋₌₍₎");
    });
    it("to super test (DISABLED)", function () {
        return true;
        assert.equal(utils.mylib_toSuper("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-=()"), "ᴬᴮᶜᴰᴱᶠᴳᴴᴵᴶᴷᴸᴹᴺᴼᴾQᴿˢᵀᵁⱽᵂˣʸᶻᵃᵇᶜᵈᵉᶠᵍʰᶦʲᵏˡᵐⁿᵒᵖ۹ʳˢᵗᵘᵛʷˣʸᶻ⁰¹²³⁴⁵⁶⁷⁸⁹⁺⁻⁼⁽⁾");
    });
